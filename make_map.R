  ...
  function( DataList,
    ...
    if( !all(c("X1config_cp","X2config_cp") %in% names(DataList)) ){
      if( "Xconfig_zcp" %in% names(DataList) ){
        ...
        DataList$X1config_cp = array( DataList$Xconfig_zcp[1,,], dim=c(DataList$n_c,DataList$n_p) )
        DataList$X2config_cp = array( DataList$Xconfig_zcp[2,,], dim=c(DataList$n_c,DataList$n_p) )
      }else{
    ...
    if( ("OverdispersionConfig"%in%names(DataList)) && "n_v"%in%names(DataList) ){
      ...
      #ここでエラー->解除
      if( DataList[["OverdispersionConfig"]][2] == -1 ){ #合ってる
        if("L2_z"%in%names(TmbParams)) Map[["L2_z"]] = factor(rep(NA,length(TmbParams[["L2_z"]]))) #FALSEで合ってる
